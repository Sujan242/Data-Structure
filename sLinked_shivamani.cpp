...
void addNode(int x, int value)
{
	Node* temp = findNode(value);
	if(temp->next == NULL) 						//add at end
	{
		addEnd(x);
		return;
	}
	...
	temp1 = temp1->prev;
	int i = 0;									//find elements in cycle
	while(temp1->prev == NULL)
	{
		i++;
		temp1 = temp1->next;
	...
	if(k == i)                    			   	//divide the cycle
	{
		Node* prevCycleStart = temp1->prev;    	//pointer to  start of cycle 
		temp1->prev = newNode;					//previous of last of cylcle to new node
		newNode->prev = prevCycleStart;			//new node previous points to start of previous cycle
	}
